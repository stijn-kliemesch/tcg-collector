...
export class PokemonCardDemoService {
  ...
  async demonstrateFullAnalysis(imagePath: string): Promise<void> {
    ...
    const analysisReport = PokemonCardAnalyzer.formatAnalysisResults(
      result.pokemonAnalysis
    );
   ...
   */
  async demonstratePokemonNameSearch(
    imagePath: string,
    targetName: string
  ): Promise<void> {
    ...
    const result = await this.cardService.findPokemonName(
      imageBuffer,
      targetName
    );
    ...
    if (result.candidates.length > 0) {
      ...
      result.candidates.forEach((candidate, index) => {
        const ocrIcon =
          candidate.ocrConfidence >= 50
            ? '🟢'
            : candidate.ocrConfidence >= 30
              ? '🟡'
              : '🟠';
        const matchIcon =
          candidate.matchConfidence >= 40
            ? '🎯'
            : candidate.matchConfidence >= 25
              ? '🎲'
              : '🔍';

        console.log(
          `  ${index + 1}. ${matchIcon} ${ocrIcon} "${candidate.original}" → "${candidate.corrected}"`
        );
        console.log(
          `      OCR: ${Math.round(candidate.ocrConfidence)}%, Match: ${Math.round(candidate.matchConfidence)}%`
        );
        console.log(
          `      Size: ${candidate.size}, Position: ${candidate.position}\n`
        );
      });
      ...
      if (result.bestCandidate) {
        ...
        console.log(
          `    Confidence: OCR ${Math.round(result.bestCandidate.ocrConfidence)}%, Match ${Math.round(result.bestCandidate.matchConfidence)}%`
        );
      }
    } else {
  ...
  async demonstrateElementSearch(imagePath: string): Promise<void> {
    ...
    const result = await this.cardService.searchPokemonCardElements(
      imageBuffer,
      targets
    );
    ...
    for (const searchResult of result.results) {
      if (searchResult.matches.length > 0) {
        ...
        console.log(
          `✅ ${icon} ${searchResult.type.toUpperCase()}: "${searchResult.target}"`
        );
        console.log(
          `    Found: "${best.region.text}" (${confidence}% OCR, ${best.score}% match)`
        );
      } else {
        console.log(
          `❌ ${searchResult.type.toUpperCase()}: "${searchResult.target}" - NOT FOUND`
        );
      }
    }
  }
  ...
  async runAllDemos(imagePath?: string): Promise<void> {
    const defaultImagePath =
      imagePath || path.join(__dirname, '../data/example-card.png');

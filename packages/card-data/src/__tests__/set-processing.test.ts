...
describe('SetNameCleaner', () => {
  it('should clean set names by removing annotations', () => {
    expect(SetNameCleaner.cleanSetName('Base Set — Special Set')).toBe('Base Set');
    expect(SetNameCleaner.cleanSetName('Dragon Vault — Special Set')).toBe('Dragon Vault');
    expect(SetNameCleaner.cleanSetName('McDonald\'s Collection 2013 — French print only')).toBe('McDonald\'s Collection 2013');
  });

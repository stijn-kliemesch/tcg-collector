export class PokemonCardAnalyzer {
     ...
     */
    static analyzePokemonCard(textRegions) {
        const elements = [];
        ...
        const attacks = elements.filter(e => e.type === 'attack_name' || e.type === 'attack_damage');
        const setNumber = elements.find(e => e.type === 'set_number');
        const energyCosts = elements.filter(e => e.type === 'energy_cost');
        const otherElements = elements.filter(e => !['pokemon_name', 'hp', 'attack_name', 'attack_damage', 'set_number', 'energy_cost'].includes(e.type));
     ...
     */
    static searchForTargets(textRegions, options) {
        const results = [];
        for (const target of options.targets) {
            const matches = this.findTextMatches(textRegions, target.text, target.type)
     ...
     */
    static findPokemonNameCandidates(textRegions, targetName) {
        const candidates = [];
        for (const region of textRegions) {
            ...
            if (originalText.length >= 4) {
                const correction = this.smartCorrectPokemonName(originalText, targetName);
                if (correction.confidence >= 15) { // Reasonable threshold
                    candidates.push({
     ...
     */
    static smartCorrectPokemonName(text, targetName) {
     ...
     */
    static findTextMatches(textRegions, targetText, elementType) {
        return textRegions.map(region => {
            ...
            if (regionText.length < 2 && !['hp', 'attack_damage', 'set_number'].includes(elementType)) {
            ...
            // Fuzzy matching for similar text
                ...
                if (['hp', 'attack_damage', 'set_number'].includes(elementType)) {
                    ...
                    if (region.text.includes('/') && targetText.includes('/')) {
                        ...
                        const regionNumbers = region.text.match(/\d+/g) || [];
                        const targetNumbers = targetText.match(/\d+/g) || [];
                        const hasMatchingNumber = regionNumbers.some((num) => targetNumbers.includes(num));
                        if (hasMatchingNumber) {
                            score = 80;
                        }
                    }
                    else if (/^\d+$/.test(regionText) && targetText.includes(regionText)) {
                        score = 85;
                    }
     ...
     */
    static classifyCardElement(region) {
        ...
        if (/^\d{2,3}$/.test(text) && region.bbox.y < 300) { // Likely in upper area
            type = 'hp';
        }
        ...
        else if (text.length >= 6 && /^[A-Za-z\(\)\|]+$/.test(text) &&
            region.bbox.y < 350 && region.bbox.width > 150) {
            type = 'pokemon_name';
        ...
        else if (text.length >= 4 && text.length <= 20 && /^[A-Z]/.test(text) &&
            region.bbox.y > 300) {
            type = 'attack_name';
        }
     ...
     */
    static getBestPokemonName(candidates) {
        if (candidates.length === 0)
            return null;
        // Prefer candidates with higher match confidence and reasonable OCR confidence
        const scored = candidates.map(candidate => ({
            candidate,
            score: candidate.matchConfidence + (candidate.ocrConfidence * 0.3) +
    ...
    static formatAnalysisResults(analysis) {
        ...
        if (analysis.pokemonName) {
            lines.push(`🎭 Pokemon Name: "${analysis.pokemonName.correctedText}" (${Math.round(analysis.pokemonName.confidence)}% confidence)`);
        }
        if (analysis.hp) {
            lines.push(`❤️  HP: ${analysis.hp.originalText} (${Math.round(analysis.hp.confidence)}% confidence)`);
        }
        if (analysis.attacks.length > 0) {
            lines.push(`⚔️  Attacks: ${analysis.attacks.map(a => a.correctedText).join(', ')}`);
        }
        if (analysis.setNumber) {
            lines.push(`📦 Set Number: ${analysis.setNumber.originalText} (${Math.round(analysis.setNumber.confidence)}% confidence)`);
        }

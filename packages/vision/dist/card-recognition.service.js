...
import { PokemonCardAnalyzer } from './pokemon-card-analyzer.service.js';
/**
...
export class CardRecognitionService {
     ...
     */
    async recognizeCard(imageBuffer, options = {}) {
        if (!this.isInitialized) {
                ...
                if (data.words) {
                    for (const word of data.words) {
                        ...
                        if (word.confidence >= minConfidence && word.text.trim().length > 0) {
                            ...
                            const isDuplicate = allTextRegions.some(existing => Math.abs(existing.bbox.x - newRegion.bbox.x) < 10 &&
                            ...
                            if (!isDuplicate) {
                                ...
                                console.log(`   Existing duplicate:`, allTextRegions.find(existing => Math.abs(existing.bbox.x - newRegion.bbox.x) < 10 &&
                                    Math.abs(existing.bbox.y - newRegion.bbox.y) < 10 &&
                                    existing.text.toLowerCase() === newRegion.text.toLowerCase()));
                            }
                        }
     ...
     */
    async detectIcons(imageBuffer, options) {
     ...
     */
    shouldGroupWords(baseWord, candidateWord, currentGroup) {
        // Protect important Pokemon names from being over-grouped
        const isPokemonName = (text) => text.length >= 6 && /^[A-Za-z\(\)]+$/.test(text);
     ...
     */
    calculateHorizontalOverlap(bbox1, bbox2) {
        const left1 = bbox1.x;
     ...
     */
    combineTextFragments(fragments) {
     ...
     */
    detectLanguagesFromText(textRegions) {
     ...
     */
    async recognizePokemonCard(imageBuffer, options = {}) {
     ...
     */
    async searchPokemonCardElements(imageBuffer, targets, options = {}) {
        const recognitionResult = await this.recognizeCard(imageBuffer, options);
        const searchOptions = {
            targets: targets.map(t => ({ text: t.text, type: t.type, required: t.required })),
     ...
     */
    async findPokemonName(imageBuffer, targetName, options = {}) {

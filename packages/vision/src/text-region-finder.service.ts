...
export class TextRegionFinder {
  ...
  async findTextRegions(
    ...
    if (cardBounds) {
      console.log(
        `📐 Card detected at: ${cardBounds.x},${cardBounds.y} size: ${cardBounds.width}x${cardBounds.height} (confidence: ${Math.round(cardBounds.confidence * 100)}%)`
      );
    } else {
    ...
    if (config.enableLayoutAnalysis) {
      ...
      const layoutResult = await this.analyzeCardLayout(
        imageBuffer,
        cardBounds
      );
  ...
  private async detectTextAreas(
    ...
    console.log(
      `🃏 Using card bounds: ${cardOffsetX},${cardOffsetY} size: ${cardWidth}x${cardHeight}`
    );
  ...
  async analyzeCardLayout(
    ...
    console.log(
      '🖼️ Creating enhanced text detection buffer optimized for Pokemon cards...'
    );

    // Multiple preprocessing strategies for different text types from Garganacl card analysis
    // Strategy 1: For black text with white borders (most common: "Garganacl", "202/182", "Land Crush", etc.)
    const whiteBorderedText = await image
      .clone()
      .grayscale()
      .modulate({ brightness: 1.05, saturation: 1.0 }) // Very slight brightness boost
      .linear(1.02, -5) // Very gentle contrast adjustment to preserve borders
      .blur(0.3) // Tiny blur to soften harsh edges while keeping structure
      .sharpen(0.7) // Conservative sharpening
      .toBuffer();

    // Strategy 2: For solid/high contrast text ("160", "Weakness", "Ability")
    const solidText = await image
      .clone()
      .grayscale()
      .normalise() // Auto-adjust contrast
      .linear(1.15, -18) // More aggressive contrast for clear text
      .sharpen(1.1)
      .toBuffer();

    // Strategy 3: For faded/low quality text (flavor text, copyright, etc.)
    const fadedText = await image
      .clone()
      .grayscale()
      .modulate({ brightness: 1.2, saturation: 1.0 }) // Brighter for faded text
      .linear(1.3, -25) // High contrast to bring out faded text
      .sharpen(1.5)
      .toBuffer();

    // Combine all strategies: take the best result from each
    const combined = await sharp(whiteBorderedText)
      .composite([
        { input: solidText, blend: 'lighten' }, // Get clear text
        { input: fadedText, blend: 'overlay' }, // Overlay to enhance faded parts
      ])
      .threshold(115) // Lower threshold to catch more text variations
      .negate() // Text becomes white
      .toBuffer();

    const textBuffer = combined; // Step 3: Analyze the buffers to find text density areas
  ...
  > {
    ...
    console.log(
      `📐 Using grid size: ${gridSize}px for ${width}x${height} image`
    );
    ...
    for (let y = 0; y <= height - gridSize; y += stepSize) {
      for (let x = 0; x <= width - gridSize; x += stepSize) {
        ...
        if (totalPixels > 0) {
          ...
          // Enhanced thresholds for Pokemon card text characteristics
          // Based on Garganacl card analysis: black text with white borders, red text, white on red, etc.
          if (density > 0.12 && density < 0.88) {
            // Wider range for diverse text styles
            // Additional text-like characteristics check
            const isTextLike = this.hasTextLikeCharacteristics(
              imageData,
              x,
              y,
              gridSize,
    ...
    console.log(
      `📊 Sampled ${sampledCells} cells, found ${textCells} with text (${Math.round((textCells / sampledCells) * 100)}%)`
    );
    ...
    if (areas.length > 0) {
      console.log(
        `📏 Sample density areas: first area is ${areas[0].width}x${areas[0].height}, density: ${areas[0].density.toFixed(3)}`
      );
      if (areas.length > 1) {
        console.log(
          `📏 Last area is ${areas[areas.length - 1].width}x${areas[areas.length - 1].height}, density: ${areas[areas.length - 1].density.toFixed(3)}`
        );
      }
    } else {
      console.log(
        `⚠️ No density areas found! This might be due to threshold being too high (${0.08})`
      );
    }
    ...
    const textLikeRegions = areas.filter(
      area =>
        area.density > 0.25 &&
        area.density < 0.75 && // Text sweet spot (was 0.25-0.95)
        area.width >= 20 &&
        area.height >= 20 // Reasonable size for text
    );

    console.log(
      `🎯 Found ${textLikeRegions.length} text-like regions (density 0.25-0.75)`
    );

    // If we have a reasonable number of text regions (8-40), use them
    if (textLikeRegions.length >= 8 && textLikeRegions.length <= 40) {
      console.log(
        `✅ Using individual text regions approach (${textLikeRegions.length} regions in target range)`
      );
    ...
    if (textLikeRegions.length > 40) {
      console.log(
        `🔗 Too many text regions (${textLikeRegions.length}), trying smart clustering by card zones...`
      );
      const clustered = this.clusterRegionsByCardZones(
        textLikeRegions,
        width,
        height
      );
      ...
      console.log(
        `🎯 Zone clustering outside range, trying smaller regions filter...`
      );
      const smallerRegions = textLikeRegions.filter(
        area => area.width <= 120 && area.height <= 120 // Focus on smaller, more text-like areas
      );
      console.log(
        `🎯 Filtering to smaller regions: ${smallerRegions.length} regions`
      );
      ...
      const merged = this.mergeAdjacentAreasImproved(
        textLikeRegions,
        gridSize,
        stepSize,
        width,
    ...
    if (textLikeRegions.length < 8) {
      console.log(
        `� Too few text regions (${textLikeRegions.length}), trying broader criteria...`
      );
      const broaderRegions = areas.filter(
        area =>
          area.density > 0.2 &&
          area.density < 0.8 && // Slightly broader
          area.width >= 18 &&
          area.height >= 15 // Slightly smaller minimum
      );
  ...
  private mergeAdjacentAreasImproved(
    ...
    console.log(
      `🔗 Merging ${areas.length} areas with grid=${gridSize}, step=${stepSize}`
    );
    ...
    for (let i = 0; i < areas.length; i++) {
      ...
      while (changed) {
        ...
        for (let j = 0; j < areas.length; j++) {
          ...
          const shouldMerge = cluster.some(clusterArea => {
            ...
            if (
              wouldBeWidth > maxTextBlockSize ||
              wouldBeHeight > maxTextBlockSize
            ) {
              // Debug first few rejections to see what's happening
              if (cluster.length < 5) {
                console.log(
                  `🚫 Rejected merge: would create ${wouldBeWidth}x${wouldBeHeight} (max: ${Math.round(maxTextBlockSize)})`
                );
              }
              return false;
            }
      ...
      if (cluster.length > 50) {
        console.log(
          `🔍 Large cluster: ${cluster.length} areas merging to ${mergedWidth}x${mergedHeight} (max: ${Math.round(maxSize)})`
        );
      }

      if (
        mergedWidth >= minSize &&
        mergedHeight >= minSize &&
        mergedWidth <= maxSize &&
        mergedHeight <= maxSize
      ) {
        const avgDensity =
          cluster.reduce((sum, a) => sum + a.density, 0) / cluster.length;
        ...
        console.log(
          `✅ Added region: ${mergedWidth}x${mergedHeight} from ${cluster.length} areas (max: ${Math.round(maxSize)})`
        );
      } else {
        console.log(
          `⚠️ Skipping region: ${mergedWidth}x${mergedHeight} from ${cluster.length} areas (constraints: ${minSize}-${Math.round(maxSize)})`
        );
      }
    }
  ...
  private async preprocessRegionForOCR(
    ...
    switch (regionName) {
      ...
      case 'pokemon-name':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(200, region.width * 3)),
            kernel: 'cubic',
          }) // Increased scaling
          .normalize()
      ...
      case 'hp-number':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(120, region.width * 4)),
            kernel: 'cubic',
      ...
      case 'hp-text':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(80, region.width * 5)),
            kernel: 'cubic',
          }) // Much more scaling
          .normalize()
      ...
      case 'evolves-from':
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(120, region.width * 2)),
            kernel: 'cubic',
          })
          .normalize()
          .linear(1.3, -15)
      ...
      case 'ability-label':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(100, region.width * 4)),
            kernel: 'cubic',
          }) // More scaling for small text
          .normalize()
      ...
      case 'ability-name':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(150, region.width * 2.5)),
            kernel: 'cubic',
          }) // More scaling
          .normalize()
      ...
      case 'ability-text':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(200, region.width * 1.5)),
            kernel: 'cubic',
          })
          .normalize()
      ...
      case 'attack2-name':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(150, region.width * 2.8)),
            kernel: 'cubic',
          }) // More scaling
          .normalize()
      ...
      case 'attack2-damage':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(80, region.width * 3)),
            kernel: 'cubic',
          })
          .normalize()
          .linear(1.2, -10)
      ...
      case 'attack2-text':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(150, region.width * 1.8)),
            kernel: 'cubic',
          })
          .normalize()
          .linear(1.2, -10)
      ...
      case 'energy-cost-general':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(60, region.width * 4)),
            kernel: 'cubic',
          })
          .normalize()
      ...
      case 'retreat-label':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(100, region.width * 3.5)),
            kernel: 'cubic',
          }) // More scaling for small labels
          .normalize()
      ...
      case 'retreat-value':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(80, region.width * 4)),
            kernel: 'cubic',
          }) // More scaling
          .normalize()
      ...
      case 'card-type':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(100, region.width * 3)),
            kernel: 'cubic',
          }) // More scaling
          .normalize()
      ...
      case 'set-number':
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(100, region.width * 2.5)),
            kernel: 'cubic',
          })
          .normalize()
          .linear(1.2, -10)
      ...
      default:
        ...
        pipeline = pipeline
          .resize({
            width: Math.round(Math.max(100, region.width * 1.5)),
            kernel: 'cubic',
          })
          .normalize()
          .sharpen(0.8)
  ...
  private async detectCardBounds(
    ...
    try {
      ...
      if (
        cardBounds &&
        this.validateCardBounds(cardBounds, imageWidth, imageHeight)
      ) {
        return cardBounds;
      }
      ...
      console.log(
        '🔄 First attempt failed, trying alternative edge detection...'
      );
      ...
      if (
        altCardBounds &&
        this.validateCardBounds(altCardBounds, imageWidth, imageHeight)
      ) {
      ...
      console.log(
        '🎯 Trying heuristic card detection based on typical positioning...'
      );
      const heuristicBounds = this.detectCardUsingHeuristics(
        imageWidth,
        imageHeight
      );
      ...
      if (
        heuristicBounds &&
        this.validateCardBounds(heuristicBounds, imageWidth, imageHeight)
      ) {
  ...
  private async findLargestRectangle(
    ...
    if (edgePixelCount < 100 || edgePixelRatio > 0.3) {
      console.log(
        `⚠️ Edge detection failed: ${edgePixelCount} pixels, ratio: ${edgePixelRatio.toFixed(3)}`
      );
      return null;
    }
    ...
    console.log(
      `📐 Detected card bounds: ${cardBounds.x},${cardBounds.y} size: ${cardBounds.width}x${cardBounds.height}`
    );
    return cardBounds;
  }
  ...
  private validateCardBounds(
    ...
    if (Math.abs(aspectRatio - expectedAspectRatio) > aspectRatioTolerance) {
      console.log(
        `⚠️ Invalid aspect ratio: ${aspectRatio.toFixed(3)} (expected ~${expectedAspectRatio.toFixed(3)})`
      );
      return false;
    }
    ...
    if (areaRatio < 0.2 || areaRatio > 0.95) {
      console.log(
        `⚠️ Invalid size ratio: ${areaRatio.toFixed(3)} (expected 0.2-0.95)`
      );
      return false;
    }
    ...
    console.log(
      `✅ Card bounds validation passed (aspect: ${aspectRatio.toFixed(3)}, area: ${areaRatio.toFixed(3)})`
    );
    return true;
  }
   ...
   */
  private detectCardUsingHeuristics(
    imageWidth: number,
    imageHeight: number
  ): CardBounds | null {
    ...
    for (const marginPercent of [0.05, 0.1, 0.15, 0.2]) {
      ...
      const aspectScore =
        1 - Math.abs(aspectRatio - expectedAspectRatio) / expectedAspectRatio;

      if (aspectScore > bestScore && aspectScore > 0.7) {
        // At least 70% match
    ...
    if (bestBounds) {
      console.log(
        `🎯 Heuristic detection found card: ${bestBounds.x},${bestBounds.y} size: ${bestBounds.width}x${bestBounds.height} (score: ${bestScore.toFixed(3)})`
      );
      return bestBounds;
    }
  ...
  private clusterRegionsByCardZones(
    ...
    for (const zone of zones) {
      ...
      const regionsInZone = regions.filter(region => {
        ...
        return (
          regionCenterX >= zoneX &&
          regionCenterX <= zoneX + zoneWidth &&
          regionCenterY >= zoneY &&
          regionCenterY <= zoneY + zoneHeight
        );
      });
      ...
      if (regionsInZone.length > 0) {
        ...
        const mergedRegion = {
          ...
          height: maxY - minY,
          density:
            regionsInZone.reduce((sum, r) => sum + r.density, 0) /
            regionsInZone.length,
          confidence:
            regionsInZone.reduce((sum, r) => sum + (r.confidence || 0.8), 0) /
            regionsInZone.length,
        };

        // Only keep reasonably sized merged regions
        if (
          mergedRegion.width <= cardWidth * 0.95 &&
          mergedRegion.height <= cardHeight * 0.4
        ) {
    ...
    console.log(
      `🏷️ Zone clustering: ${regions.length} → ${clusteredRegions.length} regions`
    );
    return clusteredRegions;
  }

  /**
   * Enhanced text-like characteristics detection for Pokemon card text
   * Based on Garganacl card analysis: various text colors, borders, backgrounds
   */
  private hasTextLikeCharacteristics(
    imageData: Buffer,
    x: number,
    y: number,
    gridSize: number,
    ...
    // Sample edge pixels to detect text borders (common in Pokemon cards)
    let edgeTransitions = 0;
    let edgeSamples = 0;

    const sampleStep = Math.max(2, Math.floor(gridSize / 8));

    // Check horizontal edges (top and bottom of cell)
    for (let dx = 0; dx < gridSize; dx += sampleStep) {
      const topX = x + dx;
      const bottomX = x + dx;
      const topY = y;
      const bottomY = y + gridSize - 1;
      ...
      if (
        ...
        // Check top edge
        if (topY > 0) {
          const currentPixel = imageData[topY * width + topX];
          const abovePixel = imageData[(topY - 1) * width + topX];
          if (Math.abs(currentPixel - abovePixel) > 64) edgeTransitions++;
          edgeSamples++;
        }

        // Check bottom edge
        if (bottomY < height - 1) {
          const currentPixel = imageData[bottomY * width + bottomX];
          const belowPixel = imageData[(bottomY + 1) * width + bottomX];
          if (Math.abs(currentPixel - belowPixel) > 64) edgeTransitions++;
          edgeSamples++;
        }
      }
    }

    // Check vertical edges (left and right of cell)
    for (let dy = 0; dy < gridSize; dy += sampleStep) {
      const leftX = x;
      const rightX = x + gridSize - 1;
      const leftY = y + dy;
      const rightY = y + dy;
      ...
      if (
        ...
        // Check left edge
        if (leftX > 0) {
          const currentPixel = imageData[leftY * width + leftX];
          const leftPixel = imageData[leftY * width + (leftX - 1)];
          if (Math.abs(currentPixel - leftPixel) > 64) edgeTransitions++;
          edgeSamples++;
        }

        // Check right edge
        if (rightX < width - 1) {
          const currentPixel = imageData[rightY * width + rightX];
          const rightPixel = imageData[rightY * width + (rightX + 1)];
          if (Math.abs(currentPixel - rightPixel) > 64) edgeTransitions++;
          edgeSamples++;
        }
      }
    }
    ...
    // Text characteristics based on density and edge patterns
    if (density >= 0.12 && density <= 0.35) {
      // Low density text (like outlined text) - need good edges
      return edgeRatio > 0.15;
    } else if (density >= 0.35 && density <= 0.65) {
      // Medium density text (solid text) - moderate edge requirements
      return edgeRatio > 0.08;
    } else if (density >= 0.65 && density <= 0.88) {
      // High density text (thick/bold text) - lower edge requirements

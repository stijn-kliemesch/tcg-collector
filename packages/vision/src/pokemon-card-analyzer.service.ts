import type { RecognizedText, CardRecognitionResult, PokemonCardElement } from '@tcg-collector/api-types';
...
export class PokemonCardAnalyzer {
  ...
  ): Array<{ target: string; type: string; matches: Array<{ region: RecognizedText; score: number }> }> {
    const results = [];
   ...
   */
  static smartCorrectPokemonName(text: string, targetName?: string): { corrected: string; confidence: number } {
   ...
   */
  private static classifyCardElement(region: RecognizedText): PokemonCardElement | null {
